...
const AddTransactionScreen = ({route, navigation}) => {
  ...
  return (
    <KeyboardAwareScrollView style={{flex: 1}}>
      ...
      <Title style={{marginLeft: 20, marginTop: 15, marginBottom: 10}}>
        Paid By:
      </Title>
      <View style={{flexDirection: 'row'}}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (paidBy < route.params.memberNames.length - 1) {
              setPaidBy(paidBy + 1);
              paidByListRef.scrollToIndex({index: paidBy + 1});
            } else {
      ...
      />
      <Title style={{marginLeft: 20, marginBottom: 10}}>Split Bill By:</Title>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (!checkingMandatories()) {
                navigation.push('MembersSplit', {
                  title,
                  date: date.toDateString(),
                  category: DefaultTheme.categories[category],
                  paidBy: memberNames[paidBy],
                  price: price,
